...
対象 URL は `https://www.reddit.com/r/<subreddit>/comments/<投稿ID>/[<slug>]` 形式とする。ホスト名の `www` は省略可、末尾スラッシュの有無は問わない。それ以外の Reddit URL（サブレディット一覧・ユーザーページ等）は対象外とし、Reddit バックエンドを構成しない。
