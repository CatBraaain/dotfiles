...
| タイミング | 動作 |
| --- | --- |
| セッション開始時 | 設定を読み込み候補を評価する |
| `/new`・`/reload` 後 | 設定と待機状態を読み直し、自動ルーティングを再開する |
| 各プロンプト送信前 | 候補を再評価する |

切替確認（候補が現在のモデルと違うときだけ表示）:

| 操作 | 結果 |
| --- | --- |
| 承認 | 選んだモデルへ切り替え、成功時に切替先を通知 |
| キャンセル | 現在のモデルを維持、自動切替を停止して `manual` 状態へ |
| 切替先に API キーがない | 切り替えず警告を表示 |
