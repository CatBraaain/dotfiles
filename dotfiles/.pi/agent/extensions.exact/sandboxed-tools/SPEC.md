...
`${REPOSITORY_NAME}` は main worktree の basename へ展開する。セッション cwd が linked worktree でも main worktree 基準で解決し、Git repository 外ではパスを許可・bind・作成しない。`~/projects/worktrees/${REPOSITORY_NAME}` のようにパスの任意の位置に記述でき、展開後のパス配下だけが write の allow および read-write bind の対象となる。
