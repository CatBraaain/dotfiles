...
セッションは **agent** を実行する。agent は利用できるツール、依頼できる子 agent、システムプロンプトを持ち、モデル候補の順序（class）の既定値を持つ実行主体である。本拡張は agent 定義の管理と選択を行い、`/agent:<name>` コマンド・`--agent` フラグ・subagent ツールの `agent` パラメータを提供する。class は `/class` コマンドと `--class` フラグで agent と独立に選択できる。

モデルの自動選択と切り替えはこの拡張だけが行う。ユーザーは `agents.yaml` で class と agent を管理し、`/agent:<agent>` で実行する agent を、`/class` でモデル候補の順序を切り替えられる。
...
| `classes.<name>`            | class のモデル候補の配列。配列の先頭から fallback 順序として評価する                                                                                                                                                                                                                                                                                                                    |
...
| `tools`                     | pi 標準ツールと拡張機能の allowlist。指定したツールを実行し、未指定のツールを拒否してセッションを継続する。`["*"]` はすべてのツール、`[]` はツールなしを許可する。`"!<tool-name>"` は `"*"` を含んでもそのツールを除外する。`["*", "!<tool-name>"]` は有効である。                                                                                                                      |
...
| `/agent:<name> [message]`   | 指定した agent を即時に有効にする。モデルは切替先 agent の既定 class の候補を適用し、ツール・agent 表示も切り替える。`message` を続けた場合は agent 切り替え完了後にそのテキスト（前後の空白を除く）をユーザーメッセージとして送信し、切替先 agent の設定で直ちにターンを開始する。`message` を省略した場合は切り替えのみ行う                                                           |
| `/class [name]`             | 実効 class（後述）を `name` に切り替え、手動選択を解除し、切り替え先 class の候補を適用する。`name` を省略した場合は選択 UI で class を選ぶ。選択 UI をキャンセルした場合は何も変更しない。UI を持たない環境で `name` を省略すると usage エラーを通知して何も変更しない。表示は「通知のない環境」の規定に従う。未定義の `name` は warning で通知し、何も変更しない。cooldown は維持する |
| `--class <name>` フラグ     | 初期 class（後述）を指定する。実効 class はセッション開始・`/new`・セッション切替・分岐でこの class に初期化される。未定義の値は warning で通知し、無視する。`--agent` と独立であり併用できる                                                                                                                                                                                           |

## 設定の検証

起動時に設定ファイルを検証する。次のいずれかに該当する場合はエラーを通知し、拡張のすべての機能（agent 切替、ツール制限、subagent、モデルルーティング）を無効にする。不正な設定を一部分だけ適用することはない。画像添付は例外として無効化後も有効にせず、Vision 入力を作らないエラーを返す。

- 設定ファイルがない・読めない・YAML として不正
- `default` か `agents` がない、型が違う
- `agents` の各定義がオブジェクトでない
- agent の `class` がない、文字列でない
- agent が `classes` に定義されていない class を参照している
...
モデル候補の選択に使う class（実効 class）は、`/class` で切り替えた class を優先し、切り替えていないときは agent の `class` を既定値とする。実効 class は agent の選択とは独立であり、画像入力対応を含むモデルの種類による制限をしない。

セッション開始・`/new`・セッション切替・分岐のタイミングでは、`--class` フラグで有効な class が指定されていれば実効 class はその class になり、指定がなければ初期 agent の既定 class になる。これを初期 class と呼ぶ。
...
| class 内の全候補が不成立、または全候補で適用に失敗 | 「モデルの適用タイミング」の該当行に従う |
...
| タイミング                                 | 動作                                                                                                                                                                                     |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| セッション開始                             | 初期 agent（`--agent` フラグで指定された定義済みの agent、指定なしまたは未定義なら `default`）の初期 class の候補を適用する                                                              |
| `/new`                                     | 手動選択を解除し、cooldown をすべて破棄し、初期 class の候補を適用する                                                                                                                   |
| セッション切替・分岐（`/resume`・`/fork`） | 手動選択を解除し、実効 class を初期 class に戻し、その候補を適用する                                                                                                                     |
| `/reload`                                  | 設定を読み込み直す。手動選択状態・実効 class・cooldown は維持する。保存済み agent・class が reload 後の設定に存在しない場合は、agent は初期 agent へ、class は agent の既定 class へ戻す |
| 各プロンプト送信前（自動状態のとき）       | 現在の実効 class の候補を再評価し、現在のモデルと異なる候補が成立したら切り替える。全候補が不成立の場合はエラーを通知し、プロンプトを実行しない                                          |
| `/agent:<name>`                            | agent を切り替え、手動選択を解除し、実効 class を切替先 agent の既定 class に戻してその候補を適用してから後続メッセージを送信する                                                        |
| `/class [name]`                            | 実効 class を切り替え、手動選択を解除し、切り替え先 class の候補を適用する                                                                                                               |
...
stateDiagram-v2
    ...
    手動 --> 自動: /agent で agent 切替
    手動 --> 自動: /class で class 切替
```

| 状態 | 振る舞い                                                               |
| ---- | ---------------------------------------------------------------------- |
| 自動 | 各プロンプト送信前に実効 class の候補を再評価する                      |
...
除外: z-ai モデルの同時実行系エラー（`1302`/`1305`、判定は `shared/zai-concurrency.ts` の `isZaiConcurrencyLimited`）は、文言がこのパターンに一致しても fallback を行わない。担当は `concurrency-retry` 拡張（`concurrency-retry/SPEC.md`）。
...
- 現在の実効 class の候補数を N とすると、1ターンで最大 N-1 回再試行し、最大 N 候補を順に試行する。再試行回数は `~/.pi/agent/settings.json` の `retry.maxRetries`（候補数以上を推奨）が上限となる
...
| 画像入力非対応 | `vision` を子 agent として起動し、画像と依頼を渡す。親には子の最終テキストと成果物だけを返す |

チャット貼り付けと CLI の `@file` で画像が入力された場合、active agent のモデルが画像入力対応なら、画像をそのセッションのモデルへそのまま入力する。画像非対応なら、画像をモデルへ送らず、画像を含む子セッションを `vision` として起動し、親へ返すのは子の最終テキストと成果物だけとする。agent 設定が無効な場合、または `vision` を起動できない agent で画像が入力された場合は、画像をモデルへ送らずエラーを通知する。

画像を処理できるモデルがない場合（`vision` への委譲が class 候補の不成立で失敗した場合や、`vision` が画像非対応モデルを選択している場合を含む）は、画像を画像非対応モデルへ送るフォールバックを行わず、画像を処理できないエラーを返す。
...
subagent ツールは子セッションを起動するとき `--agent <name>` に加えて、子セッションのセッション保存先と表示名（後述）を渡し、モデルと `--class` を指定しない。子セッションの実効 class は指定された agent の既定 class から始まり、候補を選んで適用する。子セッションでもこの spec のモデルルーティング（自動選択、429 フォールバック）が同じように働く。
...
| ---------------------- | ------------------------------------------------------------ |
| スキル自動発見         | 親と同じルール                                               |
| モデル                 | 指定された子 agent の既定 class から、子セッションが解決する |
...
子セッションはエフェメラルにせず、メインセッションと同じ保存ツリーの `subagents/` へ保存する。

`<project-key>` は Pi 本体と同じ cwd エンコード方式で決める。cwd の先頭の `/` または `\\` を除き、`/`・`\\`・`:` を `-` に置換し、前後を `--` で囲む。

| 項目   | 振る舞い                                                                                                               |
| ------ | ---------------------------------------------------------------------------------------------------------------------- |
| 保存先 | `~/.pi/agent/sessions/<project-key>/subagents/`。子セッションの cwd ごとに分け、拡張のロード時にディレクトリを用意する |
...
子 pi は `--mode json` で起動し、stdout の JSON イベントを親が読み取る。子は stdout への書き込みが読み取られない間（バックプレッシャー）は agent loop の進行を止めるため、親は子の実行中ずっと stdout を読み続けなければならない。

表示の更新（onUpdate の送出）はレンダリングを同期実行する重処理になり得る。子が bash 等で大量のイベントを短時間に流したとき、イベントごとに表示を更新すると親のイベントループがレンダリングで占有され、stdout の読み取りが止まり、子の agent loop も停止する。これを防ぐため、イベントの受領・記録は毎回行い、onUpdate の送出は 0.15 秒に 1 回までに間引く。

## subagent 実行中の表示フォーマット

subagent の呼び出し時と実行結果を、次の形式で表示する。実行結果は subagent ツールの結果欄に子エージェントごとの実行ブロックとして表示する。折りたたみは行わない。子からのイベント（`message_end`・ツール実行の開始・更新・終了）は毎回受領して記録する一方、表示の更新は 0.15 秒に1回までに間引き、間引き中のイベントは最新の状態に併合する。子の実行確定時には必ず最新状態で更新する。
...
| 更新          | 子からのイベントは毎回受領・記録し、表示は 0.15 秒に1回まで間引く。間引き中のイベントは最新の状態に併合する。子の実行確定時には必ず最新状態で更新する |
...
| ツール           | コール行                                                                                                                          | 結果行（成功時）                                  |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| `bash`           | `→ $ <command>`（300 文字で切り詰め）                                                                                             | 実行秒数（例: `1.2s`）。計測できないときは `done` |
...
| 切り替え   | 0.5 秒間隔でフレーム文字を切り替える。行を増やさず同じ位置で更新する。再描画のたびに本体が画面の全コンポーネント行を再構築するため、高速な切替は巨大セッションでメインスレッドを占有する。間隔は titlebar と共通の `SPINNER_INTERVAL_MS` を使う |
...
| 無進捗タイムアウト | 子の stdout から parse 可能な JSON イベントを最後に受信してから 15 分間、新たなイベントがなければ子を停止し、subagent ツールの結果をエラーとして親へ返す。tool 実行中（`tool_execution_start` 受信から対応する `tool_execution_end` 受信まで）は計測しない。ツールの実行時間はツール側の時間管理に任せる。停止理由は `stalled`、エラーメッセージは `Child stalled: no child events for 15 min` とする                                                                                                                                                                |
| 子の異常終了       | 子プロセスの終了コードが 0 以外、シグナルによる終了、子がエラーで停止した場合、または終了コード 0 で最終アシスタントテキストが空の場合、subagent ツールの結果をエラーとして親へ返す。結果は `Child <停止理由 \| failed>: <出力>`の形式で、出力は子のエラーメッセージ・stderr・最終アシスタントテキストの順に採用し、末尾を優先して最大2000文字にする。超過時は末尾に `[Output truncated; showing the tail.]` を付ける。いずれもなければ`(no output)`とする。シグナルによる終了の停止理由は`killed`、エラーメッセージは `Child process was killed by a signal` とする |
