...
| `/agent:<name> [message]`   | 指定した agent を即時に有効にする。モデルは切替先 agent の tier の候補を適用し、ツール・agent 表示も切り替える。`message` を続けた場合は agent 切り替え完了後にそのテキスト（前後の空白を除く）をユーザーメッセージとして送信し、切替先 agent の設定で直ちにターンを開始する。`message` を省略した場合は切り替えのみ行う |
...
モデルレジストリの `Model.input` が `"image"` を含むモデルを画像入力対応、含まないモデルを画像非対応とする。tier の候補配列を先頭から順に評価し、最初に成立した候補を選ぶ。

| 候補の状態 | 扱い |
| --- | --- |
| モデルがモデルレジストリに存在しない | 除外して次へ |
| active agent が main で、モデルが画像入力対応 | 除外して次へ |
| active agent が visual_agent で、モデルが画像入力非対応 | 除外して次へ |
...
| タイミング                                 | 動作                                                                                                                                                |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| セッション開始                             | 初期 agent（`--agent` フラグで指定された定義済みの agent、指定なしまたは未定義なら `default`）の tier の候補を適用する                              |
...
| `/agent:<name>`                            | agent を切り替え、手動選択を解除し、切替先 agent の tier の候補を適用してから後続メッセージを送信する                                               |
...
| 全候補が不成立になる場面                                      | 結果                                                                                                |
| ------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| セッション開始・`/new`・セッション切替・分岐・`/agent:<name>` | 現在のモデルを維持し、`no available model for agent <name>: tier <tier-name>` を warning で通知する |
...
active agent が main の間、`/model` による手動選択は画像入力非対応モデルだけを受け付ける。画像入力対応モデルを選択した場合はエラーを通知し、現在のモデルを維持する。
...
`visual_agent` は、テキスト専用モデルを使う agent から視覚作業を分離する agent である。`tier` には画像入力をサポートするモデルだけを設定する。main は画像入力非対応モデルを使い、画像を扱うためにモデルを切り替えない。
...
`visual_agent` が active の間、`/model` による手動選択は、モデルレジストリで画像入力対応と判定される `vision` tier の候補だけを受け付ける。tier 外または画像入力非対応のモデルを選択した場合はエラーを通知し、現在のモデルを維持する。これにより、`visual_agent` を直接選択した場合を含め、画像は `vision` tier の候補にだけ送られる。
...
画像入力をサポートする候補が `vision` tier にない、またはすべて適用できない場合、画像をテキスト専用モデルへ送るフォールバックは行わず、画像を処理できないエラーを返す。
...
subagent ツールは子セッションを起動するとき `--agent <name>` に加えて、子セッションのセッション保存先と表示名（後述）を渡し、モデルを指定しない。子セッションは指定された agent の tier から候補を選んで適用する。子セッションでもこの spec のモデルルーティング（自動選択、429 フォールバック）が同じように働く。
...
子セッションはエフェメラルにせず、`~/.pi/agent/subagent-sessions/` へ保存する。メインのセッション保存先（`~/.pi/agent/sessions/`）へは保存しない。

| 項目   | 振る舞い                                                                                                                |
| ------ | ------------------------------------------------------------------------------------------------------------------------ |
| 保存先 | `~/.pi/agent/subagent-sessions/`。拡張のロード時にディレクトリを用意する                                                |
| 表示名 | `<agent>: <task の先頭行の先頭 30 文字>`。31 文字目以降は `…` に切り詰める。先頭行が空のときは `<agent>` のみとする     |
| 保持   | 拡張のロード時に、更新日時が 30 日を超えた `.jsonl` ファイルを削除する                                                  |
| 再開   | `pi --session-dir ~/.pi/agent/subagent-sessions -r` で保存済みの子セッションを選んで再開できる                           |
