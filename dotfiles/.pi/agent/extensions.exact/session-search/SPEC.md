...
`since` / `until` に不正な ISO date を指定した場合はエラーを返す。日付のみ（`YYYY-MM-DD`）の指定はその日全体を含む解釈とする（`since` は日の始まりから、`until` は日の終わりまで、いずれもローカルタイム）。
...
| L2（予備）  | L1 と同じ正規化を施した上で、クエリの文字が順に出現すればヒット（fzf風） | 0件をそのまま返す   |
...
| トークン数 | セッション内で totalTokens を報告した最後の usage のコンテキスト総トークン数（実測）。そのような usage がないセッション（API 呼び出しが一度もない等）ではこの行を表示しない |
...
| あり  | 省略          | 検索周辺     | query のヒット箇所の前後2メッセージ                 |
| 省略  | あり          | 範囲指定     | offset から limit 件                                 |
| あり  | あり          | 検索周辺     | query のヒット箇所の前後2メッセージを offset・limit でページネーションする |
...
全文・範囲指定・検索周辺モードとも、戻り値が limit を超えると切り詰め、「全N件中 X〜Y件を表示。続きは offset=Z」の形式で次の開始位置を案内する。長大セッション（数千メッセージ）でも、これを繰り返せば全件を読める。一度の呼び出しで全文を返して呼び出し元のコンテキストを圧迫することはない。

検索周辺モードで複数ヒット時は、ヒット箇所ごとに前後2メッセージを順に返し、サイズ上限で切れたら続きをページネーションする。検索周辺モードの offset・limit は、ヒット箇所の前後を含めて構成したメッセージ列へ適用する。
...
```
## 2026-08-07 /home/username/projects/dotfiles — <id>
[user] 中間レイヤーの出現 kwsk
[assistant] Claude Agent SDK でワークフローを...
```

先頭行は `<作成日(YYYY-MM-DD)> <cwd> — <id>` の形式である。
...
| get で offset が候補範囲を超える | ヘッダに続けて「指定位置にメッセージなし」を返す |
