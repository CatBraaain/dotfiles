...
| あり  | あり          | 検索周辺     | query のヒット箇所の前後数メッセージを offset・limit でページネーションする |
...
検索周辺モードで複数ヒット時は、ヒット箇所ごとに前後数メッセージを順に返し、サイズ上限で切れたら続きをページネーションする。検索周辺モードの offset・limit は、ヒット箇所の前後を含めて構成したメッセージ列へ適用する。
