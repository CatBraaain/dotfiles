...
name: oss-mirror
description: >-
  OSS ライブラリの実装・処理を確認するとき、~/mirrors に git clone して
  fetch で最新化した実リポジトリの、ドキュメントもソースも実ファイル
  として grep/read で追う。ドキュメントはウェブ（公式ドキュメントサイト
  ・README サイト等）ではなくクローン先のファイルだけを読む。
  web_fetch / web_search は使わない。ユーザーが OSS の実装・処理・
  ソース・動きを見たい・確認したいと言った場合は常に使用する。
---
