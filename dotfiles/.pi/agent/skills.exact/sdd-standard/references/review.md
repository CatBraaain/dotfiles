...
既存のテストと実装が spec に適合しているかをレビューする。出力はギャップレポート（指摘のみ）で、テストは実行しない。
...
sdd-standard SKILL.md の「振る舞いの照合単位」に従い、spec の視覚的構造を振る舞いの単位へ分解する。
...
- **[coverage / 漏れ / 高]** spec「空文字/@なし → 状態400・invalid_email」— 対応するテストが存在しない。
- **[fidelity / 主語ズレ / 高]** 既存メール登録テスト — spec は「状態409・エラーコード email_taken」を要求するが、テストは状態409のみ検証し email_taken を含まない。Then の具体値が欠落。
- **[fidelity / 偽陽性 / 高]** 既存メール登録テスト — バックエンドをモックで409固定にしており、実装のエラーマッピング経路を叩いていない。通っても実装の振る舞いを検証していない。
- **[余剰 / 余剰実装 / 高]** `register()` — 登録成功時に確認メールを送信するが、spec にこの振る舞いの記述が無い。
- 残りの振る舞いは適合。
