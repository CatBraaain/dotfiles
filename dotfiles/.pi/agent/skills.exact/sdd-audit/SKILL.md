...
3者（spec / テスト / 実装）の間を、矢印の向きごとに3つの軸で照合する。
...
| **逆指摘** | 実装・テスト → spec | spec は一意か・網羅的か | 曖昧 / 欠落 |

spec を第一 oracle とするが、実装/テスト側から spec の曖昧さ・欠落も逆指摘する（両方向）。spec が静的で完璧であることは前提としない — 監査の過程で仕様書側の不備も浮かび上がる。
...
- **欠落（informational）**: 実装やテストが存在するが、spec に対応する振る舞い記述が無い。

過剰（A）との切り分け: 実装/テストの意図が明白で spec 側に書き漏れと思える場合は欠落（逆指摘）、テストが実装詳細に過干渉しているだけなら過剰（A）。どちらも informational。
...
| **欠落** | 逆指摘 | informational | 実装/テストがあるが spec に記述が無い |
...
- [ ] 過剰（coverage）のうち、spec 不備（逆指摘）に切り分けるべきものを見逃していないか
- [ ] 指摘に場所と種別と重要度が付いているか
