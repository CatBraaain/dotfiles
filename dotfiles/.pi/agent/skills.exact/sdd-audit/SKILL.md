...
name: sdd-audit
description: >-
  既存のテストと実装が spec と一致しているかを監査する スキル — spec を第一 oracle とし、テストの coverage（未カバー振る舞い/過剰検証/順序ズレ）、fidelity（偽陽性リスク）、実装起点の余剰（spec 外実装）を軸で照合する。実装やテストからの逆指摘で spec 側の不備も検出する。指摘のみ出し、テストも本番コードも編集しない。review, audit, 監査, 適合性, テストは十分か, 仕様と実装のズレ, SDD（spec driven development）の検証フェーズ。
...
spec を第一 oracle とするが、実装/テスト側から spec の曖昧さも逆指摘する（両方向）。spec が静的で完璧であることは前提としない — 監査の過程で仕様書側の不備も浮かび上がる。「実装にあって spec にない」は余剰（C）で実装起点に網羅的に拾う。
...
いずれかが欠けていれば、監査ではなくその旨を返して止まる。実装詳細（関数/クラス名・DB・エンドポイント）は spec 作成時点で切り捨て済みのため、spec に書かれていないものを推測で補完しない。足りなければ質問するか、情報不足として指摘する。
...
実装を読んで抽出した観測可能な振る舞い（前述「実装の分解」）が、spec に対応する記述を持つかを問う。**spec 起点の coverage（A）では「spec にない実装」は見えない。この実装起点の照合を独立して必ず回すことが、spec 外実装の1回検出に直結する。**

- **余剰実装（blocking）**: 実装にある観測可能な振る舞いが spec に書かれていない。
...
spec を第一 oracle とするが、監査の過程で以下が見えた場合は spec 側の不備として指摘する:
