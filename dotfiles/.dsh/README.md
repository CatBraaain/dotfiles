...
通常の run script は target path の辞書順で apply の途中に実行され、`run_after_` 付きは全ターゲットの適用後に実行される。ビルド（`run_after_build.sh`）は after で実行するため、plugin の `src/` が全て展開されてから bundle される。profile の `run_bun_install.sh` による再リンクはビルドに先行するが、`file:` 依存は per-file シムリンクで中身を常に最新に見せるため問題ない。
