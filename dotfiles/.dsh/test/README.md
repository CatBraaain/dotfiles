...
| 4. footer — composer.dock | dsh 標準 `Button`（ghost / sm）で `session: <id>` ラベル + コピーアイコン、composer 下 dock | 書式とアイコン。標準 Button の見た目（`--dsw-alias-button-*` token）で描けること。隣の StatsPills（stock）と縦に整列し、水平中央寄せ |
...
- **UI primitives**: shell が提供する `@deepseek-ai/dsh-client-ui-primitives` は raw npm package のままでは bun で動かないため、fixture は同一 DOM 形状の stub + 実アイコン path で置き換え、実物の `Button.module.css`（devDependencies のインストール物から読む）を当てる
...
- 静的 render のため subscription・インタラクションは動かない（snapshot の初期値のみ反映）。footer のクリックコピー（clipboard 書き込みとチェックアイコンへの切替）も実 dsh 起動後の確認対象
