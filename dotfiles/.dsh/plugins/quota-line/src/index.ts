...
function discoverZaiProvider(ctx: PluginContext): { ref?: string; baseUrl?: string } {
  try {
    ...
    const llm = settings?.get?.("llm-pi-ai") as
      | {
          providers?: Record<
            string,
            { apiKeyEnv?: string; apiKeyEnvRef?: string; baseUrl?: string }
          >;
        }
      | undefined;
    ...
    for (const [id, provider] of Object.entries(providers)) {
      if (
        !/zai|z\.ai|bigmodel|glm/i.test(id) &&
        !/zai|z\.ai|bigmodel|glm/i.test(String(provider?.baseUrl ?? ""))
      )
        continue;
      return { ref: provider?.apiKeyEnv ?? provider?.apiKeyEnvRef, baseUrl: provider?.baseUrl };
    }
  } catch {
    /* settings namespace absent — fall back to well-known refs */
  }
  return {};
}
...
export function assertZaiPayload(payload: unknown): void {
  ...
  if (root.success !== true && root.code !== 200) {
    throw new Error(
      `zai quota endpoint rejected the query (code ${String(root.code ?? "n/a")}): ${String(root.msg ?? "unknown error")}`,
    );
  }
}
...
async function fetchZai(ctx: PluginContext): Promise<ZaiResult> {
  ...
  if (key === undefined) {
    throw new Error(
      "zai credential not found: no zai/bigmodel provider in settings, and no well-known ref resolved",
    );
  }
  const payload = await fetchJson(zaiQuotaUrl(baseUrl), {
...
export function parseCodexGrant(payload: unknown): CodexGrant | null {
  ...
  const stored =
    typeof record.accountId === "string" && record.accountId !== "" ? record.accountId : null;
...
function refreshFailure(body: Record<string, unknown>): { detail: string; reauth: boolean } {
  ...
  if (nested !== null && typeof nested === "object" && !Array.isArray(nested)) {
    ...
    const code =
      typeof nestedRecord.code === "string" && nestedRecord.code !== ""
        ? nestedRecord.code
...
async function refreshCodexGrant(refreshToken: string): Promise<CodexGrant> {
  ...
  if (!response.ok || body === undefined) {
    const failure =
      body === undefined
        ? { detail: "unreadable response body", reauth: false }
        : refreshFailure(body);
    if (failure.reauth) {
      throw new Error(
        `codex credential rejected (HTTP ${response.status}, ${failure.detail}) — sign in again via dsh to re-enable this row`,
      );
    }
...
async function resolveCodexGrant(ctx: PluginContext): Promise<CodexGrant | undefined> {
  ...
  if (
    store === undefined ||
    typeof store.readRecord !== "function" ||
    typeof store.modifyRecord !== "function"
  ) {
    return undefined;
  }
  ...
  inFlight = (async () => {
    try {
      ...
      if (providers.length === 0) {
        throw new Error(
          `no quota provider available (${Object.entries(errors)
