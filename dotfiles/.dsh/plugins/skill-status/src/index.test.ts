...
describe("isSuccessfulToolResult", () => {
  ...
  it("treats an error identity or error block as failed", () => {
    assert.equal(
      isSuccessfulToolResult(toolResult("c1", { error: { name: "Error", code: "skill" } }).data),
      false,
    );
...
describe("skillStatusProjectionDefinition", () => {
  ...
  it("skips malformed skill arguments without state changes", () => {
    ...
    const next = definition.apply(
      state,
      logged({ type: "tool/call", data: { callId: "c1", name: "skill", arguments: "broken" } }, 1),
    );
  ...
  it("keeps the same state reference for unrelated and duplicate events", () => {
    ...
    assert.equal(
      definition.apply(state, { type: "turn/start", seq: 1, time: 0, data: {} } as never),
      state,
    );
    assert.equal(
      definition.apply(state, {
        ...
        seq: 2,
        time: 0,
        data: { message: { content: [], source: { callId: "unknown" } } },
      } as never),
      state,
    );
