...
- `chezmoi apply`、`just apply` など、オーナーが実行する反映操作が必要な場合は、完了報告の `## 補足` に次の形式で1行だけ記載する。
  - 形式: `反映: オーナーが <command> を実行`
  - 例: `反映: オーナーが just apply を実行`
- 「実行していない」「勝手に実行しなかった」などの経緯や理由は記載しない。
- オーナーによる反映操作が不要な場合は、この項目を省略する。
